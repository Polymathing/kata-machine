...
const { updateWeeklyAlgorithms, updatePracticeHistory } = require("./srs-scheduler");
...
algorithmsToPractice.forEach(ds => {
    const item = dsa[ds];
    if (!item) {
...
align.package_json({ dsa: algorithmsToPractice }, relative_day_path);
align.stats({ dsa: algorithmsToPractice }, day_path);
